...
def extractPlate(imgOriginal, listOfMatchingChars):
    ...
    intPlateWidth = int((listOfMatchingChars[-1].intBoundingRectX +
                         listOfMatchingChars[-1].intBoundingRectWidth -
                         listOfMatchingChars[0].intBoundingRectX) * PLATE_WIDTH_PADDING_FACTOR)

    intTotalCharHeights = sum([char.intBoundingRectHeight for char in listOfMatchingChars])
    fltAverageCharHeight = intTotalCharHeights / len(listOfMatchingChars)
    ...
    fltCorrectionAngleRad = math.asin(fltOpposite / fltHypotenuse)
    fltCorrectionAngleDeg = fltCorrectionAngleRad * (180.0 / math.pi)

    possiblePlate.rrLocationOfPlateInScene = (tuple(ptPlateCenter), (intPlateWidth,intPlateHeight), fltCorrectionAngleDeg)
    
    rotationMatrix=cv2.getRotationMatrix2D(tuple(ptPlateCenter),fltCorrectionAngleDeg,1.0)
