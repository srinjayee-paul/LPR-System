...
def preprocess_image(img):
    """ Improve grayscale to text conversion by enhancing preprocessing """
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # Convert to grayscale
    gray = cv2.GaussianBlur(gray, (5, 5), 0)  # Apply Gaussian Blur
    gray = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)  # Adaptive thresholding

    kernel = np.ones((3, 3), np.uint8)  # Morphological kernel
    gray = cv2.morphologyEx(gray, cv2.MORPH_CLOSE, kernel)  # Closing operation to fill gaps

    return gray

def extract_text_from_plate(plate_image):
    """ Extracts text from a processed plate image using Tesseract OCR """
...
def process_image(image_path):
    ...
    if len(listOfPossiblePlates) == 0:
        ...
        preprocessed_plate = preprocess_image(licPlate.imgPlate)

        # Extract text using Tesseract OCR
        licPlate.strChars = extract_text_from_plate(preprocessed_plate)

        cv2.imshow("Processed Plate", preprocessed_plate)
        ...
        cv2.imshow("Final Image", imgOriginalScene)
...
def drawRedRectangleAroundPlate(imgOriginalScene, licPlate):
    try:
        ...
        cv2.polylines(imgOriginalScene, [p2fRectPoints], isClosed=True, color=SCALAR_RED, thickness=2)
